...
- 🔶 **school-students.ts (22 endpoints, being sub-sliced — mounted /api/v1/school-admin):**
  ...
  - ▶ remaining writes: **FM-066** verify community-service + review change-request (both enum writes; review has a conditional student_course_plan create + studentInCallerSchool) → **FM-067/068** the SES-entangled invites (student invite/resend/bulk-onboard + parent invite/unlink/resend). **DECISION (Federico lean): keep the actual email-SENDS in Node-polyglot (like AI/Bedrock); port only cleanly-separable DB-write halves.** AI (ai-suggest/ai-import) stays Node permanently.
