...
- ▶ **NEXT Phase-B:** remaining `school.ts` reads that need a co-flip (school/profile GET+PUT, settings GET+PUT, users GET, isams status/jobs GET) → then `school-courses.ts` (396, read/write/AI-entangled — sub-slice around Bedrock ai-import) → `school-students.ts` roster reads. Then the school write surfaces. Medium risk (school-scoped).
