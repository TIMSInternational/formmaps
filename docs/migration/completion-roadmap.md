...
- **W** question360 writes (FM-042, folded + ready to ship) — POST/PUT/activate/deactivate/DELETE/bulk-create; single-flag `FORMMAPS_ROUTE_QUESTION360_TO_DOTNET` gates the whole question360 surface.
- **W** 🔴 token-gated external write rail (capstone, SCOPED) — evaluation submit-feedback + vocationalTake submit + violations; new fail-closed non-tenant RLS-bypass rail. Spec: scratchpad/token-rail-capstone-spec.md.
