...
- 🔶 **school-students.ts (22 endpoints, being sub-sliced — mounted /api/v1/school-admin):**
  ...
  - ▶ remaining: **parents reads** (/parents [caller-school grouping+stats], /students/:id/parents [studentInCallerSchool bypass + Date.now()/TimeProvider expired/pending status]) → **course-plan reads** (/course-plan, /change-requests, /course-request-deadline GET) → then the **14 writes** (SES-entangled invites/resend, bulk-onboard preview+commit, delete [soft], verify community-service, review change-request, parent invite/unlink/resend). AI (ai-suggest/ai-import) stays Node permanently.
