...
- ▶ **NEXT Phase-B:** remaining `school.ts` — iSAMS integration reads (status/jobs GET) + writes (configure/sync/test POST — vendor), staff-invite POST (needs FM-045 SES `ses:SendEmail` prod role), logo POST (multipart→S3). Then `school-courses.ts` (396, read/write/AI-entangled — sub-slice around Bedrock ai-import) → `school-students.ts` roster reads/writes. Medium risk (school-scoped).
