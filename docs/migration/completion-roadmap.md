...
- ▶ **NEXT Phase-B:** `school.ts` leftovers still in Node — **iSAMS WRITE side (configure/sync/test — the vendor boundary, defer until courses+students land)**, staff-invite POST (needs FM-045 SES `ses:SendEmail` prod role), logo POST (multipart→S3). Recommended sequencing: **`school-courses.ts`** (396, read/write/AI-entangled — sub-slice around Bedrock ai-import) → **`school-students.ts`** roster reads/writes → THEN the deferred iSAMS+courses+students-dependent writes. Medium risk (school-scoped).
