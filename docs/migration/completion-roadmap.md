...
- ▶ **NEXT Phase-B:** remaining `school.ts` — `/users` GET (listSchoolUsers) + `/users/:id/grade-level` PUT, iSAMS integration reads (status/jobs GET) + writes (configure/sync/test POST — vendor), staff-invite POST, counselor assign/unassign POST/DELETE, logo POST. Then `school-courses.ts` (396, read/write/AI-entangled — sub-slice around Bedrock ai-import) → `school-students.ts` roster reads/writes. Medium risk (school-scoped).
