...
- 🔶 **school-courses.ts mini-phase (27 endpoints, being sub-sliced):**
  ...
  - ▶ **data-mappings (FM-056, spec written, queued)** — GET+POST /data-mappings + POST /bulk-approve (collision-free literals); PUT/DELETE /:id defers (ai-suggest collision). Ratified safe divergence: bulk-approve no-ids → 0 (legacy Prisma drops filter → approves ALL). confidence Decimal→string.
  - ▶ remaining after FM-056: prerequisites, course-sequence/pathways, bulk course import; PUT/DELETE /courses/:courseId + /data-mappings/:id (need a batched negative-lookahead rewrite to exclude the Node-staying ai-suggest/ai-import siblings); AI (ai-suggest/ai-import) stays Node.
