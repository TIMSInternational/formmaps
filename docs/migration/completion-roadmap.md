...
- ▶ **NEXT Phase-B reads:** `school.ts` method-unambiguous reads (dashboard/stats, counselor-workload, counselor-assignments/all, notes), then `school-courses.ts` (396, read/write/AI-entangled — sub-slice around Bedrock ai-import), `school-students.ts`. Medium risk (school-scoped). Then the school write surfaces.
