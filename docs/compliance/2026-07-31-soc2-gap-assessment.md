...
**Priority: Medium-High** — close the .NET CI secret-scanning gap (cheap, high-leverage); get written confirmation from TIMS that the coKey was rotated.

### 6. Data Retention / Deletion Policies
**Status: partially covered — reactive, not policy-driven.**

- Node has functioning GDPR endpoints: `POST /api/v1/admin/users/:userId/export` and `DELETE /api/v1/admin/users/:userId/gdpr-delete` (`api/src/routes/admin.ts:246,260`), backed by `exportUserData`/`gdprDeleteUser` in `adminService.ts`, which spans "every tenant-scoped table for this user" and is itself audit-logged.
- **Gap:** this is admin-triggered, on-demand erasure — I found **no automated retention schedule** (e.g., a scheduled job purging stale sessions, expired assessment attempts, old audit-log rows past a defined window) and **no written retention policy document** analogous to TimsSuite's `05-data-retention-and-erasure-policy.md`. There's nothing in FormMaps' `docs/` tree defining how long data types are kept.
- The .NET side has no equivalent GDPR export/delete surface yet (consistent with Domain 9/10 not being migrated).

**Priority: Medium.** GDPR mechanics exist; a written retention schedule and a scheduled-purge job do not.
...
| Data retention/deletion | GDPR export/delete works (manual); no written policy, no automated purge | Medium |
