...
const TYPE_COLORS: Record<string, { bg: string; color: string }> = {
  ...
  meeting: { bg: "rgba(59,130,246,0.12)", color: "var(--admin-accent-blue)" },
...
export default function SessionNotesPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["sa-notes", search, typeFilter, page],
    queryFn: async () => {
          ...
          {([
            ...
            <button key={opt.value} onClick={() => setGroupBy(opt.value)}
              style={{
                ...
                background: groupBy === opt.value ? "var(--admin-accent-blue)" : "transparent",
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10 }}>
                  {groupBy === "student" && (
                    <span onClick={() => router.push(`/school-admin/users/${group.key}`)}
                      ...
                      onMouseEnter={(e) => { (e.currentTarget as HTMLSpanElement).style.color = "var(--admin-accent-blue)"; }}
              ...
              return (
                ...
                >
                  <div style={{ padding: "16px 20px" }}>
                    {/* Top row: student, counselor, type badge, date */}
                    <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10, flexWrap: "wrap" }}>
                      {/* Student name */}
                      <span
                        onClick={(e) => {
                          e.stopPropagation();
                          router.push(`/school-admin/users/${note.student.id}`);
                        }}
                        style={{
