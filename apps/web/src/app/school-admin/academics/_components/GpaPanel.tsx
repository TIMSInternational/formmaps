                    ...
                    <TableCell className="py-3 px-4">
                      <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                        <div style={{ flex: 1, height: 6, borderRadius: 3, background: "var(--admin-bg-hover)", overflow: "hidden", maxWidth: 80 }}>
                          <div style={{ height: "100%", borderRadius: 3, width: `${pct}%`, background: pct >= 90 ? "#10b981" : pct >= 75 ? "var(--admin-accent-blue)" : pct >= 50 ? "#f59e0b" : "#ef4444" }} />
                        </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(100px, 1fr))", gap: 6 }}>
            {GRADE_ORDER.map(letter => (
              <div key={letter} style={{ display: "flex", alignItems: "center", gap: 6 }}>
                <span style={{ fontSize: 12, fontWeight: 700, width: 28, color: letter === "F" ? "#ef4444" : letter.startsWith("A") ? "#10b981" : letter.startsWith("B") ? "var(--admin-accent-blue)" : letter.startsWith("C") ? "#f59e0b" : "var(--admin-font-secondary)" }}>{letter}</span>
