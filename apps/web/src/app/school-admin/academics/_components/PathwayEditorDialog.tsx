...
import {
  buildPathwayGraph, diffPrereqs, wouldCreateCycle, edgeId,
...
export function PathwayEditorDialog({ open, onClose }: PathwayEditorDialogProps) {
  ...
  useEffect(() => {
    ...
    const connectedSet = new Set(built.connectedIds);
    const initialNodes = catalog.filter((c) => connectedSet.has(c.id)).map(toNode);
    const rfEdges: Edge[] = built.edges.map((e) => ({ id: e.id, source: e.source, target: e.target, type: "prereq" }));
    setNodes(layoutTopDown(initialNodes, rfEdges));
    setEdges(rfEdges);
  }, [catalogData, catalog, catalogCourses]);
  ...
  return (
    <Dialog open={open} onOpenChange={(o) => { if (!o) requestClose(); }}>
      ...
      >
        {/* Header */}
        <DialogHeader className="px-5 py-3 flex-row items-center gap-3 space-y-0" style={{ borderBottom: "1px solid var(--admin-border-default)" }}>
          <DialogTitle className="flex items-center gap-2 mr-auto" style={{ color: "var(--admin-font-primary)", fontSize: 15 }}>
            <GitBranch style={{ width: 17, height: 17, color: "#065292" }} />
            Pathway editor
          </DialogTitle>
          <DialogDescription className="sr-only">
            Drag between courses to set prerequisites. Pathways are derived from these edges.
          </DialogDescription>
