...
const TYPE_COLORS: Record<string, string> = { holiday: "#ef4444", break: "#f59e0b", professional_development: "var(--admin-accent-blue)", exam: "#8b5cf6", event: "#10b981" };
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }} style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 12 }}>
        {[
          { label: t("calendar.stats.academicYear"), value: currentYear?.name || "—", icon: Star, color: "var(--admin-accent-blue)" },
              ...
              {[...Array(daysInMonth)].map((_, i) => {
                ...
                return (
                  <div key={day}
                    ...
                    <div style={{ fontSize: 12, fontWeight: isToday ? 700 : 400, color: isToday ? "var(--admin-accent-blue)" : "var(--admin-font-secondary)", marginBottom: 2 }}>{day}</div>
                    {isQuickAdd ? (
        ...
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }} style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          <div style={{ borderRadius: 10, border: "1px solid var(--admin-border-default)", background: "var(--admin-bg-card)", overflow: "hidden" }}>
            <div style={{ padding: "10px 14px", borderBottom: "1px solid var(--admin-border-default)", display: "flex", alignItems: "center", gap: 6 }}>
              <BookOpen style={{ width: 13, height: 13, color: "var(--admin-accent-blue)" }} /><span style={{ fontSize: 12, fontWeight: 600, color: "var(--admin-font-primary)" }}>{t("calendar.terms.title")}</span>
            </div>
            <div style={{ padding: 8 }}>
