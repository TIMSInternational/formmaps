...
const INTEGRATIONS: Integration[] = [
  { id: "isams", name: "iSAMS", descKey: "isams", icon: Database, status: "available", color: "var(--admin-accent-blue)", href: "/school-admin/integrations/isams" },
...
const STATUS_STYLES: Record<string, { key: string; bg: string; color: string }> = {
  ...
  available: { key: "available", bg: "rgba(59,130,246,0.1)", color: "var(--admin-accent-blue)" },
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }} style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 12 }}>
        {[
          ...
          { label: t("integrations.stats.available"), value: available.toString(), icon: Plug, color: "var(--admin-accent-blue)" },
          { label: t("integrations.stats.total"), value: integrations.length.toString(), icon: Settings, color: "var(--admin-accent-blue)" },
        ].map((s) => (
