...
// Colour palette for applications (cycles)
const DOT_COLORS = [
...
function parseDeadlineDate(deadline: string): Date | null {
  // Accept "YYYY-MM-DD", "MM/DD/YYYY", or any date-parseable string
...
export default function ApplicationsCalendarPage() {
  ...
  useEffect(() => {
    async function load() {
      try {
        setIsLoading(true);
        const res = await apiRequest<any>("/api/v1/student/applications", { method: "GET" });
        const list: TrackedApplication[] = res?.data ?? res ?? [];
  ...
  // ── Build deadline map: "YYYY-MM-DD" -> TrackedApplication[] ──────────────
  ...
  // Assign stable color index per application id
    ...
    <div className="space-y-5 max-w-5xl">
      ...
      >
        ...
        <p className="max-w-2xl text-base text-muted-foreground">
          All your application deadlines at a glance.
        </p>
      </motion.div>
        ...
        >
          ...
          >
            ...
            >
              <div className="flex items-center gap-3">
                <h2 className="text-lg font-bold" style={{ color: "var(--admin-font-primary)" }}>
                ...
                return (
                  <div
                    ...
                    style={{
                      background: isSelected
                        ...
                        : "var(--admin-bg-card)",
                      border: isSelected
                        ? "1.5px solid var(--admin-accent-blue)"
                        : todayDay
                  ...
                  >
                    ...
                    <span
                      ...
                      style={{
                        background: todayDay ? "var(--admin-accent-blue)" : "transparent",
                        color: todayDay ? "white" : hasDeadlines ? "var(--admin-font-primary)" : "var(--admin-font-tertiary)",
                      }}
                    >
                      {day.getDate()}
                    </span>

                    {/* Deadline dots / labels */}
                    {hasDeadlines && (
                      <div className="flex flex-col gap-0.5 flex-1">
                        {dayApps.slice(0, 2).map((app) => {
                          ...
                          return (
                            <div
                              key={app.id}
                              className="flex items-center gap-1 px-1 py-0.5 rounded text-[9px] font-medium leading-tight truncate"
          ...
          {applications.filter((a) => a.deadline).length > 0 && (
            <div
              ...
              style={{ background: "var(--admin-bg-card)", border: "1px solid var(--admin-border-default)" }}
            >
              <p className="text-[10px] uppercase tracking-wider font-bold mb-3" style={{ color: "var(--admin-font-tertiary)" }}>
