...
import {
  Briefcase, TrendingUp, ArrowUpRight, Building2, Search,
  MapPin, CheckCircle2, XCircle, Clock, Zap, Target, MoreHorizontal,
} from "lucide-react";
import Link from "next/link";
import { Button } from "@/components/ui/button";
import { Badge } from "@/components/ui/badge";
import {
  AreaChart, Area, Tooltip, ResponsiveContainer, BarChart, Bar, Cell,
} from "recharts";
import {
  marketStats, salaryData, roleDemandData, hotSkills,
  salaryByLevel, topCompanies, topLocations, recentJobs, item,
} from "./MarketData";
...
// ── Salary Chart Card ──
export function SalaryChartCard() {
  return (
    <motion.div
      variants={item}
    ...
    >
      <div className="absolute top-0 right-0 w-96 h-96 bg-indigo-500/20 rounded-full blur-3xl translate-x-1/3 -translate-y-1/3 group-hover:bg-indigo-500/30 transition-colors duration-700" />
      <div className="absolute bottom-0 left-0 w-64 h-64 bg-emerald-500/10 rounded-full blur-3xl -translate-x-1/3 translate-y-1/3" />
...
// ── Hiring Volume Card ──
export function HiringVolumeCard({ hoveredRole, setHoveredRole }: { hoveredRole: number | null; setHoveredRole: (v: number | null) => void }) {
  return (
    <motion.div
      variants={item}
      whileHover={{ y: -5, transition: { duration: 0.2 } }}
            ...
            <BarChart data={roleDemandData}>
              <Tooltip
                ...
                content={({ active, payload }) => {
                  if (active && payload && payload.length) {
                    return <div className="bg-slate-900 text-white text-xs py-1 px-2 rounded-lg shadow-lg">{payload[0].value?.toLocaleString()}</div>;
                  }
                  return null;
                }}
              />
        ...
        <div className="flex items-center gap-4 bg-white px-4 py-2 rounded-xl border border-slate-100 shadow-sm">
          ...
          <div className="flex items-center gap-2">
            <div className="h-2 w-24 bg-slate-100 rounded-full overflow-hidden">
              <div className="h-full bg-gradient-to-r from-indigo-500 to-violet-500" style={{ width: `${(userSkillsCount / totalSkills) * 100}%` }} />
            </div>
        ...
        {hotSkills.map((skill, i) => (
          ...
          >
            <div className="flex justify-between items-start mb-3">
              ...
              {skill.yours ? <CheckCircle2 className="w-5 h-5 text-emerald-500 bg-emerald-50 rounded-full" /> : <XCircle className="w-5 h-5 text-slate-300 group-hover:text-rose-400 transition-colors" />}
            </div>
            <div className="flex items-center justify-between">
              ...
              <span className={`text-[10px] font-bold px-2 py-1 rounded-md ${skill.yours ? "bg-white text-emerald-600 shadow-sm" : "bg-emerald-50 text-emerald-600"}`}>{skill.growth}</span>
            </div>
          </motion.div>
        ))}
      </div>
  ...
  return (
    <motion.div variants={item} className="md:col-span-4 bg-white border border-slate-200 rounded-[2rem] p-6 shadow-sm flex flex-col">
      ...
      <div className="space-y-6 flex-1 flex flex-col justify-center">
        {salaryByLevel.map((lvl, i) => (
          <div key={i} className="relative pl-4 group">
            {i !== salaryByLevel.length - 1 && (
              <div className="absolute left-[5px] top-6 bottom-[-24px] w-[2px] bg-slate-100 group-hover:bg-indigo-100 transition-colors" />
            )}
            <div className={`absolute left-0 top-1.5 w-2.5 h-2.5 rounded-full border-2 ${lvl.current ? "bg-indigo-600 border-indigo-600" : "bg-white border-slate-300 group-hover:border-indigo-300"} transition-colors z-10`} />
            <div className="flex justify-between items-center mb-1">
              <span className={`text-sm font-bold ${lvl.current ? "text-indigo-600" : "text-slate-600"}`}>{lvl.level}</span>
              <span className="text-sm font-bold text-slate-900">{lvl.range}</span>
            </div>
            {lvl.current && (
              <motion.div layoutId="currentLevel" className="absolute -right-2 -top-2">
                <Badge className="bg-indigo-600 text-white border-0 text-[10px] shadow-lg shadow-indigo-500/30">You</Badge>
              </motion.div>
            )}
            <p className="text-xs text-slate-400">{lvl.years}</p>
          </div>
        ))}
      </div>
    </motion.div>
...
// ── Sidebar Cards (Companies + Locations) ──
        ...
        <div className="space-y-3">
          {topCompanies.map((company, i) => (
            <Link href="#" key={i} className="flex items-center justify-between p-3 rounded-2xl hover:bg-slate-50 border border-transparent hover:border-slate-100 transition-all group">
              <div className="flex items-center gap-3">
                <div className={`w-10 h-10 rounded-xl flex items-center justify-center text-xs font-bold transition-transform group-hover:scale-110 ${company.color}`}>{company.logo}</div>
                <div>
...
export function CTACard() {
  return (
    <motion.div variants={item} className="md:col-span-12 relative overflow-hidden rounded-[2.5rem] shadow-2xl shadow-indigo-500/30">
      ...
      <div className="absolute bottom-0 left-0 w-[300px] h-[300px] bg-black/10 rounded-full blur-2xl translate-y-1/3 -translate-x-1/4" />
