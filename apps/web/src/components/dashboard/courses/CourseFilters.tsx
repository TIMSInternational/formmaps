    ...
    return (
      <Popover>
        <PopoverTrigger asChild>
          <Button variant="outline" className={`h-[50px] rounded-xl border-gray-200 bg-white text-gray-700 font-medium hover:bg-gray-50 justify-between min-w-[160px] ${count > 0 ? 'border-[var(--admin-accent-blue)]/30 bg-[var(--admin-accent-blue)]/10 text-[var(--admin-accent-blue)]' : ''}`}>
            <span className="flex items-center gap-2">
              {title}
              {count > 0 && (
                <Badge variant="secondary" className="bg-[var(--admin-accent-blue)]/10 text-[var(--admin-accent-blue)] hover:bg-[var(--admin-accent-blue)]/10 h-5 px-1.5 text-[10px]">
                  {count}
                </Badge>
              )}
            </span>
            ...
            {options.map((option) => (
              <div key={option} className="flex items-center space-x-2 p-1 hover:bg-gray-50 rounded-lg transition-colors">
                <Checkbox 
                  ...
                  className="border-gray-300 data-[state=checked]:bg-[var(--admin-accent-blue)] data-[state=checked]:border-[var(--admin-accent-blue)]"
                />
                <label
        ...
        <div className="flex flex-col md:flex-row gap-5">
          ...
          <div className="flex-1 relative group z-20">
            <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
              <Search className="h-5 w-5 text-gray-400 group-focus-within:text-[var(--admin-accent-blue)] transition-colors" />
            </div>
            <input
              type="text"
              placeholder={t("courses.searchCourses")}
              className="block w-full pl-11 pr-4 py-3.5 bg-gray-50 border border-gray-200 rounded-xl text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-[var(--admin-accent-blue)]/20 focus:border-[var(--admin-accent-blue)] transition-all text-base"
          ...
          <div className="min-w-[200px]">
            <Select value={sortBy} onValueChange={onSortChange}>
              <SelectTrigger className="w-full h-[50px] rounded-xl border-gray-200 bg-white text-gray-700 font-medium focus:ring-[var(--admin-accent-blue)]/20 focus:border-[var(--admin-accent-blue)] hover:bg-gray-50">
