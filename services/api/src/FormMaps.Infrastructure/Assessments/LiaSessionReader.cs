...
/// <c>getSession</c> (services/lia/lia-session-service.ts). <see cref="GetSessionAsync"/>'s lazy expiry
/// is delegated entirely to <see cref="ILiaSessionWriter.ReadWithLazyExpiryAsync"/> (Task 3's writer)
/// rather than duplicated here — a GET that always opened a writable transaction even when nothing
/// expired would be heavier than necessary, so this reader only escalates to the writer when its own
/// read detects a stale deadline. Column note: "lockedAt" is unmapped camelCase in both prod and the
/// test schema (no @map in schema.prisma) — every other column here is ordinary snake_case, same
/// pattern already established in LiaSessionWriter's StartAsync.
///
