...
/// <summary>
/// Schema-only Testcontainers Postgres harness for the school:manage reads (FM-DOTNET-050). Applies
/// schoolreads-schema.sql and pins a NON-UTC server timezone (America/New_York) — the reader emits ISO-Z
/// timestamps and must not depend on the container's local tz.
/// </summary>
public sealed class SchoolReadsDatabaseFixture : IAsyncLifetime
{
    ...
    private static string LoadSchemaDdl()
    {
        ...
        var name = assembly.GetManifestResourceNames().Single(n => n.EndsWith("schoolreads-schema.sql", StringComparison.Ordinal));
