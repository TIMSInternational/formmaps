...
/// <summary>
/// Schema-only Testcontainers Postgres harness for the school:manage roster reads (FM-DOTNET-062). Applies
/// school-students-schema.sql and pins a NON-UTC server timezone (America/New_York) — the reader emits ISO-Z
/// timestamps and must not depend on the container's local tz.
/// </summary>
public sealed class SchoolStudentsDatabaseFixture : IAsyncLifetime
{
    ...
    private static string LoadSchemaDdl()
    {
        ...
        var name = assembly.GetManifestResourceNames().Single(n => n.EndsWith("school-students-schema.sql", StringComparison.Ordinal));
