...
/// <summary>
/// Schema-only Testcontainers Postgres harness for the school:manage profile + settings reads/writes
/// (FM-DOTNET-051). Applies schoolprofile-schema.sql and pins a NON-UTC server timezone (America/New_York) — the
/// reader/writer emit ISO-Z timestamps and must not depend on the container's local tz.
/// </summary>
public sealed class SchoolProfileDatabaseFixture : IAsyncLifetime
{
    ...
    private static string LoadSchemaDdl()
    {
        ...
        var name = assembly.GetManifestResourceNames().Single(n => n.EndsWith("schoolprofile-schema.sql", StringComparison.Ordinal));
