...
/// <summary>
/// Schema-only Testcontainers Postgres harness for the test-scores read slice (FM-DOTNET-037). Applies
/// testscores-schema.sql (student_test_scores + universities + counselor_student_assignments +
/// student_parent_links) and pins a NON-UTC server timezone so the ISO-Z timestamp emission is caught.
/// </summary>
public sealed class TestScoreDatabaseFixture : IAsyncLifetime
{
    ...
    private static string LoadSchemaDdl()
    {
        ...
        var name = assembly.GetManifestResourceNames().Single(n => n.EndsWith("testscores-schema.sql", StringComparison.Ordinal));
